...
namespace
{
        ...
        static void Initialize(Napi::Env env) {
            ...
            Napi::Function func = DefineClass(
                    ...
                    JS_CLASS_NAME,
                    {
                            ...
                            InstanceMethod("showRangeBar", &UI::ShowRangeBar),
                            InstanceMethod("hideRangeBar", &UI::HideRangeBar),
                            InstanceMethod("callOnRangeBarChange", &UI::CallOnRangeBarChange),
                            InstanceMethod("onRangeBarChange", &UI::AddOnRangeBarChangeCallback),
    ...
    private:
        ...
        std::vector <Napi::FunctionReference> m_range_bar_change_callbacks{};

        bool m_is_ui_available = false;


        void AddOnRangeBarChangeCallback(const Napi::CallbackInfo &info) {
            auto listener = info[0].As<Napi::Function>();
            m_range_bar_change_callbacks.push_back(Napi::Persistent(listener));
        }

        void CallOnRangeBarChange(const Napi::CallbackInfo &info) {
            LOGD("NatUI CallOnRangeBarChange!\n");
            auto payload = info[0].As<Napi::String>().Utf8Value();
            for (const auto &callback : m_range_bar_change_callbacks) {
        ...
        void HideRangeBar(const Napi::CallbackInfo &info) {
            (void) info;
            CallVoidJNIMethod("hideRangeBar");
        }
        void ShowRangeBar(const Napi::CallbackInfo &info) {
            (void) info;
            CallVoidJNIMethod("showRangeBar");
        }
